...
# Every route below now resolves get_current_client_id and either clamps a
# client-role caller to their own client_id (reads) or requires a staff role
# outright (writes - a client should never edit their own portal branding/
# announcements/resources) - this module previously had none of that, so any
# authenticated user (including a client-portal account) could read or
# overwrite any OTHER client's settings/announcements/resources by supplying
# a different client_id.

# ── Settings ──
@router.get("/settings/{client_id}", response_model=PortalSettingRead, summary="Get portal setting")
async def get_portal_setting(client_id: uuid.UUID, svc: PortalSettingService = Depends(get_portal_setting_service), _: User = Depends(get_current_user), scoped_client_id: uuid.UUID | None = Depends(get_current_client_id)):
    enforce_client_scope(client_id, scoped_client_id)
...
async def upsert_portal_setting(client_id: uuid.UUID, payload: PortalSettingUpdate, db: AsyncSession = Depends(get_db), svc: PortalSettingService = Depends(get_portal_setting_service), _: User = Depends(get_current_user), _role: str = Depends(require_roles(*STAFF_ROLE_SLUGS))):
...
async def list_portal_announcements(
    ...
    items, total = await svc.list_announcements(
        search=params.search, client_id=effective_client_id, is_active=is_active,
...
async def create_portal_announcement(payload: PortalAnnouncementCreate, db: AsyncSession = Depends(get_db), svc: PortalAnnouncementService = Depends(get_portal_announcement_service), current_user: User = Depends(get_current_user), _role: str = Depends(require_roles(*STAFF_ROLE_SLUGS))):
...
async def get_portal_announcement(announcement_id: uuid.UUID, svc: PortalAnnouncementService = Depends(get_portal_announcement_service), _: User = Depends(get_current_user), scoped_client_id: uuid.UUID | None = Depends(get_current_client_id)):
    a = await svc.get_announcement(announcement_id)
...
async def update_portal_announcement(announcement_id: uuid.UUID, payload: PortalAnnouncementUpdate, db: AsyncSession = Depends(get_db), svc: PortalAnnouncementService = Depends(get_portal_announcement_service), _: User = Depends(get_current_user), _role: str = Depends(require_roles(*STAFF_ROLE_SLUGS))):
...
async def delete_portal_announcement(announcement_id: uuid.UUID, db: AsyncSession = Depends(get_db), svc: PortalAnnouncementService = Depends(get_portal_announcement_service), _: User = Depends(get_current_user), _role: str = Depends(require_roles(*STAFF_ROLE_SLUGS))):
...
async def list_portal_resources(
    ...
    items, total = await svc.list_resources(
        search=params.search, client_id=effective_client_id, resource_type=resource_type,
        sort_by=params.sort_by, sort_order=params.sort_order,
        offset=params.offset, limit=params.page_size,
    )
...
async def create_portal_resource(payload: PortalResourceCreate, db: AsyncSession = Depends(get_db), svc: PortalResourceService = Depends(get_portal_resource_service), current_user: User = Depends(get_current_user), _role: str = Depends(require_roles(*STAFF_ROLE_SLUGS))):
...
async def get_portal_resource(resource_id: uuid.UUID, svc: PortalResourceService = Depends(get_portal_resource_service), _: User = Depends(get_current_user), scoped_client_id: uuid.UUID | None = Depends(get_current_client_id)):
    r = await svc.get_resource(resource_id)
    enforce_client_scope(r.client_id, scoped_client_id)
    return PortalResourceRead.model_validate(r)

@router.put("/resources/{resource_id}", response_model=PortalResourceRead, summary="Update portal resource")
async def update_portal_resource(resource_id: uuid.UUID, payload: PortalResourceUpdate, db: AsyncSession = Depends(get_db), svc: PortalResourceService = Depends(get_portal_resource_service), _: User = Depends(get_current_user), _role: str = Depends(require_roles(*STAFF_ROLE_SLUGS))):
...
async def delete_portal_resource(resource_id: uuid.UUID, db: AsyncSession = Depends(get_db), svc: PortalResourceService = Depends(get_portal_resource_service), _: User = Depends(get_current_user), _role: str = Depends(require_roles(*STAFF_ROLE_SLUGS))):
